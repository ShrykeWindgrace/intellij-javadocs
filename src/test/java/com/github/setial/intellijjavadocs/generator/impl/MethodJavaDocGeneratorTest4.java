...
public class MethodJavaDocGeneratorTest4 extends LightJavaCodeInsightFixtureTestCase4 {
    ...
    @Before
    public void setUp() {
        methodJavaDocGenerator = new MethodJavaDocGenerator(getFixture().getProject());
        PsiElementFactory elementFactory = PsiElementFactory.getInstance(getFixture().getProject());
        JavaDocConfiguration settings = getFixture().getProject().getService(JavaDocConfiguration.class);
        generalSettings = settings.getConfiguration().getGeneralSettings();
        generalSettings.setMode(Mode.REPLACE);
        PsiFile psiFile = PsiFileFactory.getInstance(getFixture().getProject()).createFileFromText(
